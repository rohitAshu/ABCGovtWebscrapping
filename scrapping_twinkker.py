...
async def scrape_and_save_table_data(start_date, end_date, output):
    ...
    print_the_output_statement(output, 'Processing... Please wait for the generating the report.')
    ...
    try:
        ...
        while current_date <= end_date:
            ...
            print_the_output_statement(output, f"Generating Report on the dated  {formatted_date}...")
            ...
            try:
                await page.waitForSelector('select[name="license_report_length"]', timeout=60000)
                await page.click('select[name="license_report_length"]')
                await page.waitForSelector('select[name="license_report_length"] option', timeout=60000)
                await page.select('select[name="license_report_length"]', '100')
                await asyncio.sleep(5)
            except ElementHandleError as e:
                print_the_output_statement(output, f"No data found for {formatted_date}: {str(e)}")
            ...
            try:
                ...
                while True:
                    ...
                    if not next_button:
                        print_the_output_statement(output, f"No more data found for {formatted_date}")
        ...
        if save_folder:
            ...
            print_the_output_statement(output,
